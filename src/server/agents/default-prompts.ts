...
export const DEFAULT_AGENT_PROMPTS: Record<string, string> = {
  ...
  "frontend-dev":
    "Implement the React components defined in the architect's plan (provided in Previous Agent Outputs). A test plan is included in the architect's output — write test files alongside your components following the plan. Original request: {{userMessage}}",
  "backend-dev":
  ...
  "code-review":
    "Review all code generated by dev and styling agents for bugs, type errors, and correctness issues (provided in Previous Agent Outputs). Report issues clearly — dev agents will handle fixes. Original request: {{userMessage}}",
  security:
